...
int test_adventurer(struct gameState *game){
  ...
  */
  ...
  game->hand[0][handPos] = adventurer;
  ...
  int card1 = game->hand[0][handcnt-1];
  int card2 = game->hand[0][handcnt-2];

  if(card1 == 3 || card1 == 4 || card1 == 5){card1 = 3;}
  if(card2 == 3 || card2 == 4 || card2 == 5){card2 = 3;}
  ...
  error_count += asserttrue(card1, 3, __LINE__);
  error_count += asserttrue(card2, 3, __LINE__);
...
int main(){
  ...
  for(int i = 0; i < num_runs; i++){ //Configure num_runs for the amount of tests you want to do.
    ...
    error_count += test_adventurer(&game);
  }
